...
def main():
    ...
    log = roz.varys.init_logger("roz_client", os.getenv("ROZ_INGEST_LOG"), "ERROR")
    ...
    ingest_producer = roz.varys.producer(
        to_send=outbound_queue, configuration=outbound_cfg, log_file=os.getenv("ROZ_INGEST_LOG")
    ).start()
