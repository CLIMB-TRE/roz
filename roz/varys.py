...
class consumer(Thread):
    ...
    def __init__(self, received_messages, configuration, log_file, log_level="DEBUG", stream_offset="next"):
        ...
        self._parameters = pika.ConnectionParameters(host=configuration.ampq_url, port=configuration.port, credentials=pika.PlainCredentials(username=configuration.username, password=configuration.password))
    ...
    def __setup_exchange(self, exchange_name):
        ...
        self._channel.exchange_declare(exchange=exchange_name, exchange_type=self.exchange_type, callback=self.__on_exchange_declareok, durable=True)
    ...
    def __setup_queue(self, queue_name):
        ...
        self._channel.queue_declare(queue=queue_name, callback=q_callback, durable=True, arguments={"x-queue-type": "stream"})
    ...
    def __start_consuming(self):
        ...
        self._consumer_tag = self._channel.basic_consume(self._queue, self.__on_message, arguments={"x-stream-offset": self._stream_offset})
...
class producer(Thread):
    ...
    def __init__(self, to_send, configuration, log_file, log_level="DEBUG"):
        ...
        self._message_properties = pika.BasicProperties(content_type="json", delivery_mode=2)
    ...
    def __setup_exchange(self, exchange_name):
        self._log.info(f"Declaring exchange: {exchange_name}")
        self._channel.exchange_declare(exchange=exchange_name, exchange_type=self._exchange_type, callback=self.__on_declare_exchangeok, durable=True)
    ...
    def __setup_queue(self, queue):
        ...
        self._channel.queue_declare(queue=queue, durable=True, callback=self.__on_queue_declareok,  arguments={"x-queue-type": "stream"})
    ...
    def __send_if_queued(self):
        try:
            ...
            self._connection.ioloop.call_later(self._sleep_interval, self.__send_if_queued)
    ...
    def publish_message(self, message):
        ...
        self._channel.basic_publish(self._exchange, self._routing_key, message_str, self._message_properties, mandatory=True)
...
def init_logger(name, log_path, log_level):
    ...
    logging_fh.setFormatter(logging.Formatter('%(name)s\t::%(levelname)s::%(asctime)s:: %(message)s'))
...
class configurator:
    def __init__(self, profile, config_path=None):
        try:
            ...
            print("Configuration JSON does not appear to be valid or does not exist", file=sys.stderr)
            sys.exit(11)

        if config_obj["version"] != "0.1":
            print("Version number in the ROZ configuration file does not appear to be current, ensure configuration format is correct if errors are experienced", file=sys.stderr)
        ...
        if profile_dict:
            ...
            print(f"Varys configuration JSON does not appear to contain the specified profile {profile}", file=sys.stderr)
            sys.exit(2)
